...
# Оптимизация: DEBUG_MODE для включения/выключения cost_breakdown в usage
...
from parser_utils import (
    abbreviate_legal_entity,
    calculate_uncertainty,
    clean_and_build_markdown,
    convert_df_to_items,
    excel_to_grid_markdown,
    extract_specification_summary,
    normalize_for_match,
    sanitize_dataframe,
    secure_filename,
    split_excel_to_md_sections,
    transliterate,
)
...
PROJECTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "projects"
)
...
def migrate_legacy_storage():
    ...
    legacy_files = [
        f
        for f in os.listdir(old_storage)
        if os.path.isfile(os.path.join(old_storage, f)) and not f.startswith(".")
    ]

    if legacy_files:
        print(
            f"!!! ALERT: Found {len(legacy_files)} legacy files in storage/ - Migrating to Legacy_Archive project..."
        )
        ...
        with open(
            os.path.join(legacy_path, "project_state.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(state, f, ensure_ascii=False, indent=4)

        print(
            f"Legacy migration complete. Data is now in projects/{legacy_folder_name}"
        )
...
def update_supplier_profile(project_id: str, new_data: dict, file_name: str):
    ...
    if supplier_key not in suppliers:
        ...
        for profile in suppliers[supplier_key]["requisites_profiles"]:
            ...
            if profile["data"].get("bank_account") == current_requisites.get(
                "bank_account"
            ):
    ...
    try:
        ...
        for k, v in data.items():
            if os.path.isabs(k) or "/" in k or "\\" in k:
                ...
                if (
                    rel_key not in migrated
                ):  # Avoid overwriting if relative key already exists
                    migrated[rel_key] = v
                    has_changes = True
                else:
...
async def export_history(projectId: str):
    ...
    try:
        ...
        for row in history:
            ...
            lines.append(
                f"{ts} | {fn} | {model} | {method} | {cost} ₽ | {tokens} | {status}"
            )
...
async def save_config(request: Request):
    ...
    logger.info(
        f"Saving config: API_KEY length={len(data.get('keys', {}).get('YANDEX_API_KEY', ''))}, FOLDER_ID={data.get('keys', {}).get('YANDEX_FOLDER_ID', '')}"
    )
...
async def process_invoice(
    ...
    if file_id:
        ...
        if file_id in manifest:
            ...
            for k, v in manifest.items():
                if isinstance(v, dict) and (
                    v.get("originalName") == file_id
                    or v.get("original_name") == file_id
                ):
                    disk_name = k
        ...
        for k, v in manifest.items():
            if isinstance(v, dict) and (
                v.get("originalName") == original_name
                or v.get("original_name") == original_name
            ):
                disk_name = k
                break
    else:
    ...
    async def event_generator():
        ...
        try:
            ...
            if filename.endswith(".pdf"):
                ...
                from parser_utils import (
                    clean_and_build_markdown,
                    deskew_image,  # Оставляем для сканов
                    ocr_to_grid_markdown,
                )
                ...
                with pdfplumber.open(temp_path) as pdf:
                    ...
                    if len(first_page.chars) > 50 and not is_force_ocr:
                        from parser_utils import (
                            extract_digital_words_as_ocr,
                            extract_pdf_table_region,
                        )
                        ...
                        table_markdown = extract_pdf_table_region(
                            pages_data, pdf_path=temp_path
                        )
                        ...
                        if pages_data:
                            ...
                            first_page_lines = group_words_into_lines(
                                pages_data[0], y_threshold=5
                            )
                            ...
                            if table_start_y is not None:
                                header_ai_data = [
                                    w for w in pages_data[0] if w["y"] < table_start_y
                                ]
                            else:
                    ...
                    # --- ПУТЬ 2: СКАН / ПРИНУДИТЕЛЬНЫЙ OCR ---
                        ...
                        for i, pg in enumerate(pdf.pages):
                            ...
                            txt_res, low_res, words_res, raw_json = await ocr_yandex(
                ...
                # Оптимизация #1: Сокращаем юридические названия ПЕРЕД отправкой в LLM
                # Экономия ~567 токенов OUTPUT для 56 позиций (0.34₽)
                header_md = abbreviate_legal_entity(header_md)
                items_md = abbreviate_legal_entity(items_md)
                if footer_md:
                    footer_md = abbreviate_legal_entity(footer_md)
                ...
                with open(temp_path, "rb") as f:
                    txt_res, low_res, words_res, raw_json = await ocr_yandex(
                        base64.b64encode(f.read()).decode("utf-8"), api_key, folder_id
                    )
                h, t = ocr_to_grid_markdown(words_res)
            ...
            if not metadata_source:
                metadata_source = "Empty Document"
            ...
            if p_method == "excel_rules":
                ...
                main_doc, header_stats = await process_header_with_llm(
                    ai_data_to_pass, api_key, folder_id
                )
            ...
            final_struct = calculate_uncertainty(
                {"document": main_doc, "items": all_items}, has_low_confidence
            )
            if footer_data:
            ...
            final_struct.update(
                {
                    ...
                    "method": p_method,
                    "usage": accumulator.to_dict(include_breakdown=DEBUG_MODE),
                ...
                manifest[disk_name].update(
                    {
                        ...
                        "usage": accumulator.to_dict(include_breakdown=DEBUG_MODE),
                        "status": "READY_MD_AI"
                        if p_method in ["excel_ai", "pdf_text"]
                        else "READY_MD_OCR",
...
async def match_items_endpoint(request: Request):
    ...
    spec_dict = [
        {
            "id": s.get("id"),
            "raw": s.get("name", ""),
            "norm": normalize_for_match(str(s.get("name", ""))),
        }
        for s in spec_items
        if s.get("id")
    ]
    for item in invoice_items:
        ...
        item["match_data"] = {
            ...
            "target_name": best["raw"] if best else None,
            "score": round(best_s, 1),
            "status": "perfect"
            if best_s > 90
            else ("warning" if best_s >= 60 else "none"),
        }
...
def detect_file_method(file_path: str, filename: str) -> str:
    ...
    try:
        if lower.endswith(".csv"):
            ...
            df_detect = pd.read_excel(
                file_path, nrows=30, header=None, dtype=str, engine="xlrd"
            )
        else:
            df_detect = pd.read_excel(
                file_path, nrows=30, header=None, dtype=str, engine="openpyxl"
            )
...
async def storage_upload(
    projectId: str = Form(...), file: UploadFile = File(...), stage: str = Form("spec")
):
    ...
    if is_spreadsheet:
        try:
            ...
            unnamed_empty = [
                ...
                and (
                    df[c].astype(str).replace("", "nan").isnull().all()
                    or (df[c].astype(str).str.strip() == "").all()
                )
            ]
            if unnamed_empty:
            ...
            if effective_stage == "spec":
                try:
                    ...
                    if summary_md:
                        summary_p = get_file_path(
                            projectId, secured_name, "_summary.md"
                        )
    ...
    p_method = (
        "pdf_text"
        if final_status in ["READY_MD_OCR", "READY_MD_LOCAL"]
        and original_filename.lower().endswith(".pdf")
        else (existing.get("method", "") if isinstance(existing, dict) else "")
    )
...
async def storage_list(projectId: str):
    ...
    if os.path.exists(files_dir):
        for f in os.listdir(files_dir):
            if f.startswith(".") or f.endswith((".json", ".md")):
                continue
            ...
            files.append(
                {
                    ...
                    "method": "pdf_text"
                    if entry.get("status") == "READY_MD_LOCAL"
                    and not entry.get("method")
        ...
        if os.path.exists(h_path):
            try:
                ...
                new_history = [
                    h
                    for h in history
                    if h.get("fileName") != name and h.get("fileName") != dk
                ]
    ...
    for pid in os.listdir(PROJECTS_DIR):
        ...
        try:
            ...
            if os.path.exists(f_dir):
                state["filesCount"] = len(
                    [
                        f
                        for f in os.listdir(f_dir)
                        if os.path.isfile(os.path.join(f_dir, f))
                        and not f.lower().endswith((".json", ".md"))
                    ]
                )
            else:
                state["filesCount"] = 0
            ...
            for meta_file in ["manifest.json", "history.json"]:
                ...
                if os.path.exists(mp):
                    mtimes.append(os.path.getmtime(mp))
...
async def create_project(data: dict):
    # 1. Count current project folders for N+1 (only valid projects with state.json, excluding live-main)
    n = 0
    if os.path.exists(PROJECTS_DIR):
        dirs = [
            d
        ...
        print(f"[DEBUG] All directories in PROJECTS_DIR: {dirs}")
    ...
    print(f"[DEBUG] Generated default_name: {default_name}")
...
async def import_project(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not os.path.exists(state_file):
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid project archive: project_state.json missing",
            )
        ...
        for f in os.listdir(import_path):
            ...
            if f.lower().endswith(data_extensions):
                shutil.move(
                    os.path.join(import_path, f), os.path.join(final_files_dir, f)
                )

        # 6. Recalculate file count for the state
        actual_files = [
            f
            for f in os.listdir(final_files_dir)
            if os.path.isfile(os.path.join(final_files_dir, f))
        ]
        ...
        state["updatedAt"] = now
        if uuid_exists:
            state["createdAt"] = now  # Mark as new if it's a "copy" via import
