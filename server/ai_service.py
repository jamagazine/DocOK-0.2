...
class UsageStats:
    ...
    def add(self, label: str, model_key: str, input_tok: int, output_tok: int):
        rates = pricing.PRICING_CONFIG.get(
            model_key, pricing.PRICING_CONFIG["yandexgpt-pro"]
        )
        step_cost = (
            input_tok * rates["input_per_1k"] + output_tok * rates["output_per_1k"]
        ) / 1000.0
    ...
    def add_ocr(self, pages: int, method: str, label: str):
        ...
        method_cost = (
            config.get("table") if method == "ocr_table" else config.get("page")
        )
        ...
        self.cost_breakdown[label]["pages"] = (
            self.cost_breakdown[label].get("pages", 0) + pages
        )
    ...
    def merge(self, other: "UsageStats", prefix=""):
        ...
        for k, v in other.cost_breakdown.items():
            ...
            if "pages" in v:
                self.cost_breakdown[key]["pages"] = (
                    self.cost_breakdown[key].get("pages", 0) + v["pages"]
                )
...
async def estimate_cost_before_processing(
    text: str, num_positions: int = 0, **kwargs
) -> dict:
    ...
    base = (
        BASE_LITE_FEE
        + PRO_SETUP_FEE
        + (num_positions * PRICE_PER_ROW)
        + ((num_chunks - 1) * CHUNK_TAX)
    )
    ...
    async with httpx.AsyncClient(timeout=90.0) as client:
        ...
        for attempt in range(3):
            ...
            if resp.status_code == 429:
                ...
                print(
                    f"Yandex 429 (Initial Submit). Attempt {attempt + 1}/3. Waiting {wait_time}s..."
                )
        ...
        for attempt in range(max_polls):
            ...
            try:
                ...
                print(
                    f"Error polling Yandex Operation {op_id}: {e}. Retrying inner loop..."
                )
                await asyncio.sleep(2.0)

    raise Exception(
        f"OCR Operation {op_id} timed out after {max_polls * poll_interval}s."
    )
...
async def get_token_count(
    text: str, model_type: str, api_key: str, folder_id: str
) -> int:
    ...
    model_uri = (
        ...
        if model_type == "pro"
        else f"gpt://{folder_id}/yandexgpt-lite/latest"
    )
...
def parse_gpt_json(text: str):
    ...
    try:
        ...
        logger.error(
            f"Failed to parse LLM response. Possible truncation. Response: {text[:200]}..."
        )
        return None
...
def normalize_invoice_table(md_text: str) -> str:
    ...
    for line in data_rows:
        ...
        has_data = (
            any(c.isdigit() for c in price_val)
            or any(c.isdigit() for c in sum_val)
            or any(c.isdigit() for c in qty_val)
        )
...
def apply_math_arbitrage(json_data: dict) -> dict:
    ...
    for item in items:
        ...
        if abs(calc_total - total_doc) > 0.05:
            notes.append(
                f"Ошибка расчета: {qty} * {price} = {calc_total}, в доке {total_doc}"
            )
    ...
    if grant_total_doc > 0:
        ...
        if diff > 0.5:
            global_notes.append(
                f"⚠ Несовпадение итогов: Сумма строк ({total_accumulated:.2f}) != Итого в доке ({grant_total_doc:.2f})"
            )
        else:
...
async def extract_invoice_metadata(
    text: str, api_key: str, folder_id: str, system_prompt: str, model_type: str = "pro"
):
    ...
    try:
        ...
        raw_res, stats = await gpt_yandex(
            clean_text,
            api_key,
            folder_id,
            system_prompt,
...
async def process_chunks_with_gpt(
    full_text: str,
    api_key: str,
    folder_id: str,
    system_prompt: str,
    ...
    if context and "{supplier_name}" in system_prompt:
        system_prompt = system_prompt.replace(
            "{supplier_name}", context.get("supplier_name", "Не указан")
        )
    ...
    chunks = [
        data_lines[i : i + CHUNK_SIZE] for i in range(0, len(data_lines), CHUNK_SIZE)
    ]
    ...
    async def process_single_chunk(i, chunk):
        async with sem:
            ...
            try:
                raw_res, in_tok, out_tok = await gpt_yandex(
                    chunk_text,
                    api_key,
                    folder_id,
                    system_prompt,
                    model_type,
                    label=f"Header_Chunk_{i + 1}",
                )
    ...
    final_json = {"header": main_doc, "items": all_items, "footer": footer_data}
    ...
    yield {
        ...
        "fixes": all_fixes,
        "tokens": getattr(
            total_stats, "cost", 0.0
        ),  # Temporary placeholder if caller assumes it's cost/stats. Unused anyway.
...
def get_address_tokens(addr_str: str) -> dict:
    if not addr_str:
        return {"index": None, "house": None, "office": None}
    addr_str = addr_str.lower()
    idx = re.search(r"\b\d{6}\b", addr_str)
    # Ищем дом (д., дом, д, стр.)
    hs = re.search(
        r"\b(?:д\.|дом|д|стр\.|строение)\s*№?\s*(\d+[а-я]?|\d+\/\d+)\b", addr_str
    )
    # Ищем офис (оф., офис, каб., кв.)
    off = re.search(
        r"\b(?:оф\.|офис|каб\.|кабинет|кв\.|квартира)\s*№?\s*(\d+[а-я]?)\b", addr_str
    )
    return {
...
def check_address_anchor(addr_str: str, anchor_tokens: dict) -> str:
    ...
    if tokens["index"] == anchor_tokens.get("index") and tokens[
        "house"
    ] == anchor_tokens.get("house"):
...
def safe_parse_llm_json(response_text: str) -> dict:
    ...
    try:
        ...
        # DEBUG: Логируем raw_dict для диагностики
        import logging

        logger = logging.getLogger(__name__)
        logger.info(
            f"[DEBUG safe_parse_llm_json] raw_dict keys: {list(raw_dict.keys())}"
        )
        logger.info(
            f"[DEBUG safe_parse_llm_json] organization_name type: {type(raw_dict.get('organization_name'))}, value: {raw_dict.get('organization_name')}"
        )
        logger.info(
            f"[DEBUG safe_parse_llm_json] default_vat_rate in raw_dict: {'default_vat_rate' in raw_dict}, value: {raw_dict.get('default_vat_rate')}"
        )
        logger.info(
            f"[DEBUG safe_parse_llm_json] total_amount type: {type(raw_dict.get('total_amount'))}, value: {raw_dict.get('total_amount')}"
        )
        logger.info(
            f"[DEBUG safe_parse_llm_json] vat_amount in raw_dict: {'vat_amount' in raw_dict}, value: {raw_dict.get('vat_amount')}"
        )
        ...
        wrapped_data = {
            ...
            "vat_amount": raw_dict.get("vat_amount", "---"),
            "default_vat_rate": raw_dict.get("default_vat_rate", "---"),
            "currency": raw_dict.get("currency", "RUB"),
        }

        # DEBUG: Логируем wrapped_data
        logger.info(
            f"[DEBUG safe_parse_llm_json] wrapped organization_name type: {type(wrapped_data.get('organization_name'))}, value: {wrapped_data.get('organization_name')}"
        )
        logger.info(
            f"[DEBUG safe_parse_llm_json] wrapped default_vat_rate type: {type(wrapped_data.get('default_vat_rate'))}, value: {wrapped_data.get('default_vat_rate')}"
        )
...
async def process_header_with_llm(ocr_json, api_key: str, folder_id: str) -> dict:
    ...
    # 2. Читаем промпт
    with open(
        os.path.join(os.path.dirname(__file__), "prompts", "invoice_header_prompt.md"),
        "r",
        encoding="utf-8",
    ) as f:
    ...
    instruction = (
        "[INSTRUCTION]"
        + parts[1].replace("{markdown_payload}", markdown_payload).strip()
    )
    ...
    for f_name in ["legal_address", "postal_address"]:
        ...
        is_leak = (
            t["index"] == CLIENT_ANCHOR["index"]
            and t["house"] == CLIENT_ANCHOR["house"]
        ) or (
            t["house"] == CLIENT_ANCHOR["house"]
            and t["office"] == CLIENT_ANCHOR["office"]
        )
    ...
    if (
        not wrapped_data["postal_address"]["value"]
        and wrapped_data["legal_address"]["value"]
    ):
        wrapped_data["postal_address"]["value"] = wrapped_data["legal_address"]["value"]
        wrapped_data["postal_address"]["confidence"] = 0.5
        wrapped_data["postal_address"]["note"] = (
            "Адрес продублирован из Юридического (оригинал не найден)"
        )
    ...
    if acc_val and str(acc_val).startswith("301"):
        ...
        if found_407:
            ...
            wrapped_data["bank_account"]["note"] = (
                "Р/С автоматически найден по префиксу 407"
            )
        else:
            wrapped_data["bank_account"]["value"] = None
            wrapped_data["bank_account"]["note"] = (
                "Ошибка: в поле Р/С был К/С. Реальный Р/С не найден."
            )

    # 2. Если К/С пуст, попробуем найти его по префиксу 301
    if (
        not wrapped_data["corr_account"]["value"]
        or wrapped_data["corr_account"]["value"] == "---"
    ):
        ...
        if found_301:
            ...
            wrapped_data["corr_account"]["note"] = (
                "К/С автоматически найден по префиксу 301"
            )
    # ------------------------------------------

    from validators import validate_bank_requisites, validate_inn, validate_kpp
    ...
    wrapped_data["bank_account"] = validate_bank_requisites(
        rs_field.get("value"), "bank_account", doc_type, is_corr=False, bik=new_bik_val
    )

    # Валидация Корреспондентского счета (к/с)
    ks_field = wrapped_data.get("corr_account", {})
    wrapped_data["corr_account"] = validate_bank_requisites(
        ks_field.get("value"), "corr_account", doc_type, is_corr=True, bik=new_bik_val
    )
